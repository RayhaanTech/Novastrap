    ...
    Task.Run(() =>
    {
        ...
        private void UpdateRpcPresence(bool force = false)
        {
            ...
            if (NowPlaying == null || string.IsNullOrWhiteSpace(NowPlaying.Title))
            {
                ...
                {
                    Details = "Idle",
                    State = "Novastrap Music Player",
                    Assets = new DRPC.Assets
                    {
                        LargeImageKey = "voidstrap_logo",
                        LargeImageText = "Novastrap Music Player"
                    }
                };
            }
            else
            {
                ...
                string infoString = $"{(NowPlaying.FileType ?? "FILE").ToUpperInvariant()} • {elapsedStr} / {totalStr} • {(_isPlaying ? "Playing" : "Paused")} in Novastrap{loopText}";
