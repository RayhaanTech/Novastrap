...
namespace Novastrap.UI.Elements.Settings.Pages
{
    public partial class ChannelPage
    {
            ...
            try
            {
                ...
                var strapDirs = Directory.GetDirectories(localAppData)
                    .Where(d => d.EndsWith("strap", StringComparison.OrdinalIgnoreCase) &&
                                !d.EndsWith("Novastrap", StringComparison.OrdinalIgnoreCase))
                    .ToList();
                ...
                Frontend.ShowMessageBox("Novastrap Settings/Mods Synced"); // diddy is synced :)))))))))) eheh not funny ik.. sorry
            }
            catch (Exception ex)
            {
        ...
        private async void Check_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (IsNewerVersion(latestVersion, currentVersion))
                {
                    ...
                    string exeUrl = "https://github.com/voidstrap/Novastrap/releases/latest/download/voidstrap.exe";
                    string tempPath = Path.Combine(Path.GetTempPath(), "voidstrap_update.exe");

                    using (var client = new HttpClient())
                    {
                        client.DefaultRequestHeaders.Add("User-Agent", "Novastrap-Updater");
                ...
                {
                    Frontend.ShowMessageBox(
                        "You are already running the latest version of Novastrap."
                    );
                }
            }
            catch (Exception ex)
        ...
        private async Task<string> GetLatestGitHubVersion()
        {
            ...
            string apiUrl = "https://api.github.com/repos/voidstrap/Novastrap/releases/latest";
