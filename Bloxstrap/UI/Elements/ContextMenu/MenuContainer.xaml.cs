...
using Novastrap.UI.Chat;
using Novastrap.UI.Elements.Crosshair;
using Novastrap.UI.Elements.FPS;
using Novastrap.UI.Elements.Overlay;
using Novastrap.UI.Elements.Settings.Pages;
...
namespace Novastrap.UI.Elements.ContextMenu
{
    ...
    public partial class MenuContainer
    {
        ...
        private void MemoryTimer_Tick(object? sender, EventArgs e)
        {
            try
            {
                var voidstrapProcess = Process.GetProcessesByName("Novastrap").FirstOrDefault();
        ...
        public void ActivityWatcher_OnGameJoin(object? sender, EventArgs e)
        {
            ...
            if (App.Settings.Prop.IngameChatDiscord)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    if (App.Current.Resources["DiscordChatOverlayWindow"]
                        is Novastrap.UI.Elements.Overlay.DiscordChatOverlayWindow existing)
                    {
                    ...
                    var discordOverlay = new Novastrap.UI.Elements.Overlay.DiscordChatOverlayWindow();
                    discordOverlay.Show();
            ...
            if (App.Settings.Prop.OverlaysEnabled)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    if (App.Current.Resources["OverlayWindow"] is Novastrap.UI.Elements.Overlay.OverlayWindow existing)
                    {
                        if (existing.IsLoaded)
                        {
                            return;
                    ...
                    var overlay = new Novastrap.UI.Elements.Overlay.OverlayWindow();
                    overlay.Show();
        ...
        public void ActivityWatcher_OnGameLeave(object? sender, EventArgs e)
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                ...
                if (App.Current.Resources["OverlayWindow"] is Novastrap.UI.Elements.Overlay.OverlayWindow overlay)
                {
                    overlay.Close();
                    App.Current.Resources.Remove("OverlayWindow");
                }

                if (App.Current.Resources["DiscordChatOverlayWindow"] is Novastrap.UI.Elements.Overlay.DiscordChatOverlayWindow discordOverlay)
                {
